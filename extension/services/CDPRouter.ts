...
export class CDPRouter {
  ...
  async handleCommand(msg: ExtensionCommandMessage): Promise<unknown> {
    ...
    // Helper: list inspectable targets from Chrome
    const listChromeTargets = async () => {
    ...
    switch (msg.params.method) {
      case "DevBrowser.listTabs": {
        const targets = await listChromeTargets();
        // Return richer info (includes tabId) for convenience
        return {
          targets: targets
      ...
      case "DevBrowser.attachTab": {
        const tabId = (msg.params.params?.tabId as number | undefined) ?? undefined;
        if (!tabId || typeof tabId !== "number") {
          throw new Error("DevBrowser.attachTab requires params.tabId (number)");
        }

        // If already tracked, return existing sessionId
        const existing = this.tabManager.get(tabId);
        if (existing?.sessionId) {
          return { sessionId: existing.sessionId };
        }

        const { sessionId } = await this.tabManager.attach(tabId);
        return { sessionId };
      }
      ...
      case "Target.getTargetInfo": {
        const requestedTargetId = msg.params.params?.targetId as string | undefined;
        if (!requestedTargetId) {
          throw new Error("Target.getTargetInfo requires params.targetId");
        }

        // If we already track it, return from Chrome targets list (fresh) or from our cache.
        const targets = await listChromeTargets();
      ...
      case "Target.attachToTarget": {
        ...
        // If already tracked, return existing sessionId
        const tracked = this.tabManager.getByTargetId(targetId);
        if (tracked?.tab.sessionId) {
          return { sessionId: tracked.tab.sessionId };
        }

        // Find tabId by targetId via Chrome's target list
        const targets = await listChromeTargets();
        const found = targets.find((t) => t.id === targetId);
        const tabId = found?.tabId;
        if (!tabId || typeof tabId !== "number") {
          throw new Error(`Target ${targetId} is not a tab target (no tabId)`);
        }
